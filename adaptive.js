...
let interviewSummary = "";
  ...
  function showDeskExperiment({ editCycle = false } = {}) {
    ...
    $("#rememberForCat").closest("label").classList.toggle("hidden", COMPETITION_MODE);
    if (COMPETITION_EN) {
  ...
  function showJourneyClosing({ pending = false } = {}) {
    ...
    $(".save-choices").classList.toggle("hidden", COMPETITION_MODE);
    if (COMPETITION_MODE) document.querySelector('input[name="journeySave"][value="discard"]').checked = true;
